...
<body>
    <div class="container row">
        ...
        <div class="col">
            ...
            echo "<br><br><br>--Esimesed 3 nime nimekirjas--<br><br>";
            for ($i = 0; $i < 3; ++$i) {
                echo "$nimed[$i]<br>";
            }

            echo "<br><br><br>--Viimased 3 nime nimekirjas--<br><br>";
            for ($i = 1; $i < 4; ++$i) {
                $tempArv = (count($nimed) - $i);
                echo "$nimed[$tempArv]<br>";
            }

            echo "<br><br><br>--Suvaline nimi nimekirjas--<br><br>";
            $randInt = rand(0, count($nimed) - 1);
            echo "$nimed[$randInt]";
        ...
        <div class="col">
            ...
            printf('Autode arv on: %d<br>',count($margid));

            if (count($margid) == count($VIN)) {
                echo"Mõlemad massivid on sama suured <br><br>";
            } else {
                printf('Margid: %d   VIN: %d', count($margid), count($VIN));
            ...
            for ($i = 0; $i < count($margid); ++$i) {
                if ($margid[$i] == "Toyota") {
                    ++$toyota;
                } elseif ($margid[$i] == "Audi") {
                    ++$audi;
                }
            }
            printf("Audisi on: %d ja Toyotasid on: %d<br><br>", $audi, $toyota);

            echo"Järgmised koodid märkide arv on väiksem kui 17<br>";
            ...
            $palgadKokku = 0;
            for ($i = 0; $i < count($palgad); ++$i) {

                $palgadKokku =+ $palgad[$i];
            }

            $keskmine = $palgadKokku / count($palgad);

            printf("<br><br>Keskmine palk on: %d<br>", $keskmine);
